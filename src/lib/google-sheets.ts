 ...
 * Priority: Environment Variables (Vercel/Production)
 */
const credentials = {
...
// Use environment variables for Sheet ID and Tab Name with fallbacks for local development
const SHEET_ID = process.env.GOOGLE_SHEET_ID || '1kGU4EojCOQhvh2W4a0wH8HPS-xxwvoxejerCijUXPdY';
const TAB_NAME = process.env.GOOGLE_SHEET_TAB_NAME || 'Upcoming day requirements';
...
export async function fetchSheetData(): Promise<SheetRow[]> {
  try {
    ...
    if (!credentials.project_id || !credentials.private_key || !credentials.client_email) {
      console.warn('Missing Google Service Account environment variables.');
    }
